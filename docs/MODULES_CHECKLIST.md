...
Актуальное на 24.04.26
# MODULES_CHECKLIST.md — Карта модулей

## 1. База Данных (23 таблицы)
- **Новое**: `contact_purchases`, `payout_methods` (с частичными уникальными индексами), `claims`.
- **Обновлено**: `platform_settings` (добавлены поля `yookassa_secret_key`, `yookassa_test_mode`).

## 2. Бэкенд Сервисы
- **payouts.ts**: Логика выплат с защитой от race-condition.
- **lib/sbp-banks.ts**: Справочник 25 банков и нормализатор телефонов.
- **lib/yookassa.ts**: (Draft) Клиент для работы с платежным шлюзом.

## 3. Админ-панель
- **Вкладка «Платежи»**: Полный блок настройки ЮKassa (Shop ID, Secret, Test Mode).
- **Вкладка «Выплаты»**: Управление заявками на СБП с масками банков.