...
- **Stage 41 — Generative Infographic (3-tier fallback)** (**✅ 21.05.2026**) — AI-генерация карточек товара через Google Imagen 3. Новый файл `lib/infographic.ts`: `buildImagePrompt()` + `generateGenerativeInfographic()` + `preprocessForAI()`. 3-tier chain: Tier 1 OpenRouter (stub, cascade) → Tier 2 Imagen 3 (`generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-001:predict`) → Tier 3 graceful degradation (исходное фото) + SVG-оверлей как доп. fallback. `routes/ai.ts`: `?format=horizontal` → buildHorizontalImage (legacy OG), `?preprocess=true` → preprocessForAI перед генерацией, стандарт → generateGenerativeInfographic. `buildMarketplaceOverlaySvg` помечена `@deprecated`, `buildHorizontalImage` остаётся активным. Кэш SHA-256 24ч в `/tmp/infographic-cache/` сохранён. Логирование: Pino структурированные записи {tier, durationMs, title}. Ответ содержит `generativeTier: 1|2|3`.
