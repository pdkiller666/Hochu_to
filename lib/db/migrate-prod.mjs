...
// ════════════════════════════════════════════════════════════════════
// ПРОВЕРКА DATABASE_URL — если не задана, контейнер сразу падает с
// понятным сообщением вместо загадочного "connection refused"
// ════════════════════════════════════════════════════════════════════
if (!process.env.DATABASE_URL) {
  console.error("[migrate] FATAL: DATABASE_URL environment variable is not set!");
  console.error("[migrate] Set it in Amvera → Settings → Variables before deploying.");
  process.exit(1);
}
console.log("[migrate] DATABASE_URL: OK (set)");

// ════════════════════════════════════════════════════════════════════
// 0а. ПРЕДВАРИТЕЛЬНОЕ ЗАПОЛНЕНИЕ NULL-ЗНАЧЕНИЙ
//     Это нужно делать ДО drizzle push, иначе он попытается добавить
//     NOT NULL к колонкам с уже существующими NULL-строками и упадёт.
// ════════════════════════════════════════════════════════════════════
console.log("[migrate] Step 0a: pre-filling NULLs in existing columns");
const preClient = new Client({ connectionString: process.env.DATABASE_URL });
try {
  await preClient.connect();
  const prePatches = [
    `UPDATE bookings SET claim_status = 'none' WHERE claim_status IS NULL`,
    `UPDATE bookings SET renter_protection_enabled = false WHERE renter_protection_enabled IS NULL`,
    `UPDATE bookings SET protection_enabled = true WHERE protection_enabled IS NULL`,
    `UPDATE users   SET completed_deals_count = 0 WHERE completed_deals_count IS NULL`,
    `UPDATE users   SET is_banned = false WHERE is_banned IS NULL`,
    `UPDATE listings SET requires_manual_verification = false WHERE requires_manual_verification IS NULL`,
    `UPDATE listings SET is_available = true WHERE is_available IS NULL`,
    `UPDATE listings SET owner_protection_enabled = true WHERE owner_protection_enabled IS NULL`,
  ];
  for (const sql of prePatches) {
    try {
      await preClient.query(sql);
    } catch (_e) {
      // таблица или колонка может ещё не существовать — некритично
    }
  }
  console.log("[migrate] Step 0a: pre-fill done");
} catch (err) {
  console.warn(`[migrate] Step 0a WARNING (non-fatal): ${err.message}`);
} finally {
...
const migrations = [
  ...
  // Stage 26-B: partial unique digital_acts(pool_id, type) WHERE type='check_in'.
  // drizzle-kit push не пересоздаёт WHERE-условие partial unique → нужно явно
  // дропнуть старый индекс с условием WHERE pool_id IS NOT NULL и создать
  // новый с уточнённым условием. Иначе вторая «передача» pool_handover
  // упадёт с unique violation.
  `DROP INDEX IF EXISTS digital_acts_pool_type_uniq`,
  `CREATE UNIQUE INDEX IF NOT EXISTS digital_acts_pool_type_uniq
     ON digital_acts (pool_id, type)
     WHERE pool_id IS NOT NULL AND type = 'check_in'`,
];
